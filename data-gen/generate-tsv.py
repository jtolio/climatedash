...
def stull_wetbulb(temp_c, relhum_pc):
    ...
    return numpy.multiply(temp_c, numpy.arctan(0.151977 * numpy.float_power(numpy.add(relhum_pc, 8.313659), .5))) + \
        numpy.arctan(numpy.add(temp_c, relhum_pc)) - \
        numpy.arctan(numpy.add(relhum_pc, -1.676331)) + \
        .00391838 * numpy.float_power(relhum_pc, 1.5) * numpy.arctan(numpy.multiply(0.023101, relhum_pc)) - \
        4.686035
...
def doublecheck(ds, var, units):
    ...
    assertIn(ds.variables['time'].units, ("days since 1949-12-01 00:00:00", "days since 1949-12-01", "days since 1949-12-1 00:00:00"))
    ...
    assertEqual(ds.variables[var].shape, (
...
def wetbulb(var, units, gcm, rcm, grid, scenario, granularity, bias, timerange):
    tmean = loadfile(os.path.join(root, f"tmean.{scenario}.{gcm}.{rcm}.{granularity}.{grid}.{bias}.{timerange}.nc"))
    doublecheck(tmean, "tmean", "degC")
    relhum = loadfile(os.path.join(root, f"hurs.{scenario}.{gcm}.{rcm}.{granularity}.{grid}.{bias}.{timerange}.nc"))
    ...
    mask = numpy.any(numpy.isnan(tmean.variables['tmean']), axis=0) | \
        numpy.any(numpy.isnan(relhum.variables['hurs']), axis=0)
...
def directload(var, units, gcm, rcm, grid, scenario, granularity, bias, timerange):
    ds = loadfile(os.path.join(root, f"{var}.{scenario}.{gcm}.{rcm}.{granularity}.{grid}.{bias}.{timerange}.nc"))
...
def calc_days_above(database, varname, threshold, units, loader=directload):
    def timeframegen(timerange):
        def modelgenerator(gcm, rcm):
            data, mask, lat, lon = loader(varname, units, gcm, rcm, grid, scenario, granularity, bias, timerange)
    ...
    add_to_db("%s_days_above_%d" % (varname, threshold), database, absolutes, deltas, mask, lat, lon)
...
def calc_days_at_or_below(database, varname, threshold, units, loader=directload):
    def timeframegen(timerange):
        ...
        return ensemblemean(models[grid], modelgenerator)
    absolutes, deltas, mask, lat, lon = calc_deltas(timeframes, timeframegen)
    add_to_db("%s_days_at_or_below_%d" % (varname, threshold), database, absolutes, deltas, mask, lat, lon)
