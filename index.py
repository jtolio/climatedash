...
<p>This tool allows you to graph a variety of present and future results for the
contiguous 48 United States generated by an ensemble model
of regional climate models driven by a collection of global climate models,
provided by the <a href="https://na-cordex.org/">North American CORDEX Program</a>.
A blog post describing the methodology
<a href="https://https://www.jtolio.com/2022/07/anthropocene-calamity-part-8-climate-models-101/">can
be found on my website</a>, and please direct correspondence
to <a href="mailto:climatedash@xnet5.com">climatedash@xnet5.com</a>.</p>

<p>There are two aspects you can control. First, the color is chosen based
on the dimension or expression you select. Second, you can filter which geography
is shown by restrictions, given the full data set.</p>

<p>In <b>Simple</b> mode (above), you can choose the colored dimension from a
dropdown, and you can add and select filters from a dropdown.</p>

<p>In <b>Advanced</b> mode, you can control these using simple expressions, with
the variables listed below. Advanced and Simple mode can be switched between, as
long as the expression in Advanced mode can be represented in Simple mode.
Advanced mode can represent a greater diversity of expressions.</p>

<p>Here are the available variables and what they mean:</p>

<ul>"""
for name, var in valueChooserNames.items():
    DOCS += "\n<li><code>" + var + "</code> - " + name + "</li>"
DOCS += """</ul>

<p>Many values have a time suffix. Here are those:</p>

<ul>"""
for timename, timeext in timeChooserNames.items():
    DOCS += "\n<li><code>_" + timeext + "</code> - " + timename + "</li>"
DOCS += "</ul>"
...
def draw_docs_ui(selection_box, filter_box):
    if len(selection_box) != 1:
        selection_box = [""]
    if len(filter_box) != 1:
        filter_box = [""]
...
def draw_ui(
    ...
    if len(ui_tab) == 1 and ui_tab[0] == "tab-docs":
        selected_tab = "tab-docs"
        (within_tab, selection_expr, filter_expr) = draw_docs_ui(
            selection_box, filter_box
        )
    elif len(ui_tab) == 1 and ui_tab[0] == "tab-advanced":
    ...
    tabs.append(dbc.Tab(label="Docs", tab_id="tab-docs"))

    if selected_tab in ("tab-simple", "tab-advanced"):
        within_tab.append(
            html.Div(
